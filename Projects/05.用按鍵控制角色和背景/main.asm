...
;main.c:8: UINT8 currentspriteindex = 0 ; 
	ld	c, #0x00
;main.c:9: set_sprite_data ( 0, 2, c01);
	ld	hl, #_c01
	push	hl
	ld	a, #0x02
	push	af
...
;main.c:13: SHOW_SPRITES;
	...
	or	a, #0x02
	ldh	(_LCDC_REG+0),a
;main.c:14: set_bkg_data(0,16,backgroundtile);
	ld	hl, #_backgroundtile
	push	hl
	ld	a, #0x10
	push	af
	...
	call	_set_bkg_data
	add	sp, #4
;main.c:15: set_bkg_tiles(0,0,MAP1Width,MAP1Height,MAP1);
	ld	hl, #_MAP1
	push	hl
	ld	de, #0x1220
	push	de
...
;main.c:20: switch (joypad()){
	...
	jr	Z, 00107$
	jr	00108$
;main.c:22: case J_RIGHT:
00101$:
;main.c:23: if (currentspriteindex == 1)
	ld	a, c
...
;main.c:29: break;
	jr	00108$
;main.c:31: case J_LEFT:
00104$:
;main.c:32: if (currentspriteindex == 0)
	ld	a, c
	or	a, a
	jr	NZ, 00106$
;main.c:34: currentspriteindex = 1;
	ld	c, #0x01
00106$:
;c:/gbdk/include/gb/gb.h:1234: OAM_item_t * itm = &shadow_OAM[nb];
	ld	de, #_shadow_OAM
